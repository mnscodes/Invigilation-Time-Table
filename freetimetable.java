...
public class freetimetable extends JPanel {
	
	public freetimetable(final String a) {
	...
	existingtimetable.addActionListener(new ActionListener() {
		...
		///////// figure out to print the timetable of the lecturer try opening a new frame
			...
			Connection conn = null;
			 ...
			 String res = null;
			 String query = "select day,from2,to2 from freetimetable where username ='"+a+"'";
			try
			{
			    ...
			    //System.out.println("Connected to Oracle Database");
				 ...
				 //System.out.println(r.next());
				  JFrame tempo = new JFrame("My free Time Table");
				  ...
				  while(r.next())
				 {
					 temp[ia] = r.getString(1)+"  From: "+r.getString(2)+"  To: "+r.getString(3);			 //res=res+"\n"+temp;
					ia++;
	...
	*/
	...
	entertimetable.setText("Enter your free time table details:");
	add(entertimetable);
	...
	add(from);
	 size=from.getPreferredSize();
	    from.setBounds( 150 + i.left, 100 + i.top , 150 + size.width , 10 + size.height);
		JLabel froml = new JLabel();
		froml.setText("from: ");
		size=froml.getPreferredSize();
	    froml.setBounds( 80 + i.left, 100  + i.top , 150 + size.width , 10 + size.height);
	    add(froml);
    ...
    to.setBounds( 150 + i.left, 150 + i.top , 150 + size.width , 10 + size.height);
	JLabel tol = new JLabel();
	tol.setText("to: ");
	size=tol.getPreferredSize();
    tol.setBounds( 95 + i.left, 150  + i.top , 150 + size.width , 10 + size.height);
    add(tol);
    ...
    add.setBounds( 200 + i.left, 200  + i.top , 25 + size.width , 10 + size.height);
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try
			{
			    ...
			    //System.out.println("Connected to Oracle Database");
				 ...
				 String query = "insert into freetimetable values ('"+a+"','"+days+"','"+froms+"','"+tos+"')";
				 System.out.println(query);
	...
	ResultSet r = null;
			...
			String temp = null;
	 String res = null;
	 final String query = "delete from freetimetable where username  ='"+a+"'";
